...
def confirmvulnerableservices(path,key):
	...
	try:
		...
		for unverifiedservice in unverifiedservices:
			url = unverifiedservice
			print(colored(str(i)+':	'+url,'yellow') + "\t\tlength = "+str(len(results[url]["callbackURL"])))
			i=i+1
	except:
...
if input('Do you want to continue? y/N ') == 'y':
	...
	for serv in services:
		...
		if total_length != 0:
			path += "&additional="
			path += "".join(random.choices(string.ascii_letters, k=total_length-len(path)))
